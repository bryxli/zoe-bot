...
class TestApiGuild(unittest.TestCase):
    ...
    def setup(self, event, params=None):
        ...
        dynamo_mock.ZoeBotTable = MagicMock()
        dynamo_mock.ZoeBotTable.return_value.create_guild = MagicMock(return_value=True)

        if params == 'test_guild_exists':
            dynamo_mock.ZoeBotTable.return_value.guild_exists = MagicMock(return_value=True)
        else:
            dynamo_mock.ZoeBotTable.return_value.guild_exists = MagicMock(return_value=False)

        with patch.dict(sys.modules, {'dynamo': dynamo_mock, 'league': MagicMock()}):
